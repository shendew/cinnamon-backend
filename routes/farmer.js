...
router.post(
  ...
  [
    ...
    check(
      "date_of_planting",
      "Date of planting must be a valid date"
    ).isISO8601(),
    check("seeding_source", "Seeding source is required").not().isEmpty(),
    check("type_of_fertilizer", "Type of fertilizer is required")
      .not()
      .isEmpty(),
    check("pesticides", "Pesticides information is required").not().isEmpty(),
    check("organic_certification", "Organic certification is required")
      .not()
      .isEmpty(),
    check("expected_harvest_date", "Expected harvest date is required")
      .not()
      .isEmpty(),
    check(
      "expected_harvest_date",
      "Expected harvest date must be a valid date"
    ).isISO8601(),
    check("no_of_trees", "Number of trees is required").not().isEmpty(),
    check("no_of_trees", "Number of trees must be a valid integer").isInt({
      min: 1,
    }),
  ],
  validate,
